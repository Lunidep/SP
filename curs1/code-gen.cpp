...
int tCG::p43(){ //     BOOL -> AND
	return 0;
}

int tCG::p44(){ //      AND -> ( and ANDARGS )
	S1->obj = "(" + S3->obj + ")";
	return 0;
}

int tCG::p45(){ //  ANDARGS -> BOOL ANDARGS
	S1->obj += " && " + S2->obj;
	return 0;
}

int tCG::p46(){ //  ANDARGS -> BOOL
	return 0;
}

int tCG::p47(){ //    CPRED -> ( $idq )
	return 0;
}

int tCG::p48(){ //    CPRED -> ( $idq PDARGS )
	S1->obj = decor(S2->name) + "(" + S3->obj;
	return 0;
}

int tCG::p49(){ //   PDARGS -> ARG
	return 0;
}

int tCG::p50(){ //   PDARGS -> ARG PDARGS
	S1->obj += ",\n\t " + S2->obj + ")"; 
	return 0;
}
...
int tCG::p81(){ //   VARINI -> $int
	S1->obj = S1->name + ".";
	return 0;
}

int tCG::p82(){ //   VARINI -> $dec
	S1->obj = S1->name;
	return 0;
}
